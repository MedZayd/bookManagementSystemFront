...
const Form = ({ formik, options, open, onCloseForm, formType, onDelete }) => {
	...
	let content = (
		<Grid container spacing={2} sx={{ padding: "10px 0" }}>
			{formConfig.fields(options).map((field) => (
		...
		case type.DELETE:
			...
			content = (
				<DialogContentText sx={{ fontFamily: "Poppins" }}>
					{`Are you sure you want to delete category with ID ${values.id} ?`}
				</DialogContentText>
			);
			break;
		default:
