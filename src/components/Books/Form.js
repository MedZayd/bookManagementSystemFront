...
const Form = ({ formik, formData, open, onCloseForm, formType, onDelete }) => {
	...
	let content = (
		<Grid container spacing={2} sx={{ padding: "10px 0" }}>
			{formConfig.fields(formData).map((field) => (
		...
		case type.DELETE:
			...
			content = (
				<DialogContentText sx={{ fontFamily: "Poppins" }}>
					{`Are you sure you want to delete book with ID ${values.id} ?`}
				</DialogContentText>
			);
			break;
		default:
